...
async def test_tool_registration():
    ...
    for tool_name in expected_tools:
        assert tool_name in tools_registry, f"Tool {tool_name} not found in registry"
...
async def test_tool_functionality():
    ...
    assert python_result.output is not None
    ...
    assert terminal_result.output is not None
    ...
    session_result = await code_exec_tool.execute({
        "runtime": "output"
    })
    ...
    input_result = await input_tool.execute({
        "keyboard": "echo 'Hello from input tool'"
    })
    assert input_result.output is not None
