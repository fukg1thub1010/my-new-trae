...
3. If using Pollinations, set your key in env (recommended):

   ```bash
   export POLLINATIONS_API_KEY="your-pollinations-key"
   ```

4. **(Optional) Add mcp_servers section to enable agent to call MCP services:**
...
Trae Agent now includes three powerful tools adapted from the Agent-Zero framework:

1. **Code Execution Tool** - Executes code in multiple runtimes (Python, Node.js, terminal) with session management
2. **Input Tool** - Simulates user keyboard input and forwards it to terminal execution
3. **Search Engine Tool** - Performs web searches (with mock implementation)

These tools have been fully integrated without Docker dependencies and conform to Trae Agent's tool interface. For more details, see [AGENT_ZERO_TOOLS_INTEGRATION.md](AGENT_ZERO_TOOLS_INTEGRATION.md).
...
**MCP server init hangs:**

- Set a timeout per server in `trae_config.yaml` under `mcp_servers: <name>: timeout: <seconds>`
- Or set a global fallback env var: `TRAE_MCP_TIMEOUT=15`
- Keep `allow_mcp_servers` minimal (only servers you actually use)
...
**LLM timeouts / repeated sends:**

- Set a request timeout (seconds): `export TRAE_LLM_TIMEOUT=30`
- Ensure your model name exists at the chosen provider. Try a simpler, known-supported model.
- Lakeview is optional; if you disabled it, nothing should call Google.
